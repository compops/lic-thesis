...
########################################################################
# Run the sampler with three different step sizes
########################################################################
par.stepSize = (0.01, 0.01, 0.01);
...
pandas.DataFrame(out).to_csv("ch4-example-lgssinference-mixing-small.csv");
...
# Export data to R for plotting
out = hstack((mh.th,mh.accept))
pandas.DataFrame(out).to_csv("ch4-example-lgssinference-mixing-right.csv");

par.stepSize = (1.00, 1.00, 1.00);
mh.runSampler(kf, data, sys, thSys, par, "PMH0");
...
# Export data to R for plotting
out = hstack((mh.th,mh.accept))
pandas.DataFrame(out).to_csv("ch4-example-lgssinference-mixing-large.csv");
