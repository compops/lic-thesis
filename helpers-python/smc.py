...
#
# This file contains the SMC algorithms for filtering and smoothing 
#
########################################################################
...
class smcSampler(object):
    ...
    #############################################################################################################################
    #  Particle filtering: bootstrap particle filter for weather model
    #############################################################################################################################    
    ...
    #############################################################################################################################
    #  Particle filtering: bootstrap particle filter for Earthquake model
    #############################################################################################################################    
    ...
    def flPS(self,data,sys,par):
        ...
        if ( self.flVersion == "filtersmoother" ):
            for tt in range(0, sys.T-1):
                ...
                # Estimate score
                tmp1 = sys.Dparm(  self.p[att,tt+1], self.p[at,tt], data.y[tt], self.s[at,tt], at, data.u[tt], par);
                tmp2 = sys.DDparm( self.p[att,tt+1], self.p[at,tt], data.y[tt], self.s[at,tt], at, data.u[tt], par);
                
                osa  = sa;
                sa   = osa[bt,:] + sys.Dparm(  self.p[:,tt+1], self.p[bt,tt], data.y[tt], self.s[at,tt], at, data.u[tt], par);
        ...
        if ( self.flVersion == "full" ):
            ...
            for tt in range(0, sys.T-1):
                ...
                sal[:,:,tt+1] = sal[bt,:,tt] + sys.Dparm( self.p[:,tt+1], self.p[bt,tt], data.y[tt], self.s[bt,tt], bt, data.u[tt], par );
    ...
    def fsPS(self,data,sys,par):
        ...
        for tt in range(0, sys.T-1):
            ...
            sa  = osa[at,:] + sys.Dparm(  self.p[:,tt+1], self.p[at,tt], data.y[tt], self.s[at,tt], at, data.u[tt], par);
            sb  = osb[at,:] + sys.DDparm( self.p[:,tt+1], self.p[at,tt], data.y[tt], self.s[at,tt], at, data.u[tt], par);
